...
contract("Protocol", async (accounts) => {
  ...
  it ("tests deposit B", async () => {
    ...
    await mixerY.process_dp.sendTransaction(tx_dp, sig, {from : B});
  ...
  it ("tests preswap B to mixerY", async () => {
    ...
    await mixerY.process_sp.sendTransaction(tx_sp, sig, {from : B});
  });
  ...
  it ("tests exchange B to MixerX", async () => {
    ...
    await mixerX.process_sp.sendTransaction(tx_sp, sig, {from : B});
  });
  ...
  it ("tests B withdraw token x", async () => {
    ...
    await mixerX.process_wd.sendTransaction(tx_wd, sig);
